...
function [net, info] = train_adagrad(net, imdb, getBatch, varargin)
...
for l=numel(net.layers):-1:1
  for j=1:numel(res(l).dzdw)
    ...
    elseif (opts.useAdagrad)
      % Adagrad training
      fudge_factor = 1e-6;
      thisDecay = opts.weightDecay * net.layers{l}.weightDecay(j) ;
      thisLR = lr * net.layers{l}.learningRate(j) ;
      thisGrad = (1 / batchSize) * res(l).dzdw{j};
      if (isfield(net.layers{l}, 'hist_gradients') && length(net.layers{l}.hist_gradients) >=j)        
         net.layers{l}.hist_gradients{j} = net.layers{l}.hist_gradients{j} ...
                                       + thisGrad.^2;
      else
         net.layers{l}.hist_gradients{j} = thisGrad.^2;
      end
      net.layers{l}.momentum{j} = ...
        -thisGrad ./(fudge_factor + sqrt(net.layers{l}.hist_gradients{j})) ...
